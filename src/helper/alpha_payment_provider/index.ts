...
const alpha_payment_service = {
    create_payment: async function create_payment(internal_order_id:number, payment_order_id: number, order_amount:Decimal,ct:string) {
        ...
        return await axios.post(`${PAYMENT_URL}rest/register.do?userName=${PAYMENT_LOGIN}&password=${PAYMENT_PASSWORD}&orderNumber=${payment_order_id}&amount=${order_amount}&returnUrl=${PAYMENT_CALL_BACK_BASE_URL}/success?ct=${ct}&orderId=${internal_order_id}&failUrl=${PAYMENT_CALL_BACK_BASE_URL}/fail?ct=${ct}&orderId=${internal_order_id}`)
    },
